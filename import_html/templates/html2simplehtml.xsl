  ...
  <xsl:template name="get-content">
    ...
    <div id="content">
      ...
      <xsl:choose>
        ...
        <xsl:when test="//*[@id='milieu']">
          <xsl:attribute name="precision" >.66</xsl:attribute>
          <xsl:attribute name="title" >Found an element named 'milieu' that may be the body copy.</xsl:attribute>
          <xsl:comment>Imported From the element called milieu</xsl:comment>
          <xsl:for-each select="//*[@id='milieu']" >
            <xsl:apply-templates />
          </xsl:for-each>
        </xsl:when>
